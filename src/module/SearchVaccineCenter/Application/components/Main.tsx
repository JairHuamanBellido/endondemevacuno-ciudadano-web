...
export default function SearchVaccineCenterContainer({
  selectVaccineCenterToMap,
  vaccineCenters,
  isLoading,
  vcSelected,
  ...props
  ...
  const getFilteredVaccinationCenter = () => {
    if (!searching) return vaccineCenters;
    if (chosenSearch == 0)
      return vaccineCenters.filter((vc) =>
        vc.name.toUpperCase().includes(value.toUpperCase())
      );
    if (chosenSearch == 1)
    ...
    if (chosenSearch == 2) {
      ...
      return vaccineCenters.filter((vc) => {
        ...
        vc.vaccines.forEach((v: any) => {
          if (vaccines.includes(v)) response = true;
        });
        return response;
      });
    }
    ...
    <div className="absolute w-11/12 top-0 left-[calc(4.1%)] z-50 lg:left-0 lg:relative lg:block lg:border lg:border-r lg:border-slate-200 py-8 lg:px-6 lg:h-full lg:max-w-[500px]">
      <div className="flex bg-white lg:w-full border-2 border-slate-200 p-2 rounded-md h-14">
        {isEnteringData() && (
          <select
            value={chosenSearch}
            className="bg-primary text-white py-1.5 px-1 rounded-md focus:outline-none capitalize"
            onChange={(e) => handleChangeSearchType(+e.target.value)}
          >
            {searchTypes.map((st) => (
              <option key={`${st.name}`} value={st.id}>
                {st.name}
              </option>
            ))}
          </select>
        ...
        <input
          className="w-full focus:outline-none text-ellipsis h-9"
          placeholder={searchTypes[chosenSearch].placeholder}
          value={value}
          onChange={(e) => setValue(e.target.value)}
          readOnly={chosenSearch != 0 || hasBeenSelected()}
        />
        {value != "" && !hasBeenSelected() && (
          <div
            onClick={clearInput}
      ...
      {!isLoading && (
        <div className="bg-white px-6 py-8 mt-4 rounded-md lg:p-0 lg:m-0 lg:h-[calc(100%-100px)]">
          {(chosenSearch == 0 || searching) && !hasBeenSelected() && (
            <ListVaccineCenters
              selectVaccineCenter={selectMap}
              vaccineCenters={getFilteredVaccinationCenter()}
            />
          )}
          {chosenSearch == 1 && isEnteringData() && (
            <ListDistrics
              selectDistrict={selectDistrict}
              vaccineCenters={getFilteredVaccinationCenter()}
            />
          )}
          {chosenSearch == 2 && isEnteringData() && (
            <VaccinationList selectVaccines={selectVaccines} />
          )}
          {hasBeenSelected() && (
            <VaccineCenterDetail
              vaccineCenter={getVaccineCenterSelected()}
